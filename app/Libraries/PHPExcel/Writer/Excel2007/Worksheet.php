...
//      }
    ...
    private function writeSheetFormatPr(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
    {
        ...
        // Outline level - row
        $outlinetudiantlRow = 0;
        foreach ($pSheet->getRowDimensions() as $dimension) {
            if ($dimension->getOutlinetudiantl() > $outlinetudiantlRow) {
                $outlinetudiantlRow = $dimension->getOutlinetudiantl();
            }
        }
        $objWriter->writeAttribute('outlinetudiantlRow', (int)$outlinetudiantlRow);

        // Outline level - column
        $outlinetudiantlCol = 0;
        foreach ($pSheet->getColumnDimensions() as $dimension) {
            if ($dimension->getOutlinetudiantl() > $outlinetudiantlCol) {
                $outlinetudiantlCol = $dimension->getOutlinetudiantl();
            }
        }
        $objWriter->writeAttribute('outlinetudiantlCol', (int)$outlinetudiantlCol);
    ...
    private function writeCols(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
    {
        ...
        if (count($pSheet->getColumnDimensions()) > 0) {
            ...
            foreach ($pSheet->getColumnDimensions() as $colDimension) {
                ...
                // Outline level
                if ($colDimension->getOutlinetudiantl() > 0) {
                    $objWriter->writeAttribute('outlinetudiantl', $colDimension->getOutlinetudiantl());
                }
    ...
    private function writeSheetData(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null, $pStringTable = null)
    {
        if (is_array($pStringTable)) {
            ...
            while ($currentRow++ < $highestRow) {
                ...
                // Write current row?
                $writeCurrentRow = isset($cellsByRow[$currentRow]) || $rowDimension->getRowHeight() >= 0 || $rowDimension->getVisible() == false || $rowDimension->getCollapsed() == true || $rowDimension->getOutlinetudiantl() > 0 || $rowDimension->getXfIndex() !== null;
                ...
                if ($writeCurrentRow) {
                    ...
                    // Outline level
                    if ($rowDimension->getOutlinetudiantl() > 0) {
                        $objWriter->writeAttribute('outlinetudiantl', $rowDimension->getOutlinetudiantl());
                    }

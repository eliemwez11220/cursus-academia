    ...
    public function close()
    {
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            if (isset($columnDimensions[$columnLetter])) {
                ...
                $level = $columnDimension->getOutlinetudiantl();
        ...
        foreach ($phpSheet->getRowDimensions() as $rowDimension) {
            ...
            $this->writeRow($rowDimension->getRowIndex() - 1, $rowDimension->getRowHeight(), $xfIndex, ($rowDimension->getVisible() ? '0' : '1'), $rowDimension->getOutlinetudiantl());
        }
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
    ...
    private function writeGuts()
    {
        ...
        $maxRowOutlinetudiantl = 0;
        foreach ($this->phpSheet->getRowDimensions() as $rowDimension) {
            $maxRowOutlinetudiantl = max($maxRowOutlinetudiantl, $rowDimension->getOutlinetudiantl());
        }
        ...
        if ($maxRowOutlinetudiantl) {
            ++$maxRowOutlinetudiantl;
        }
        if ($col_level) {
            ++$col_level;
        }
        ...
        $header      = pack("vv", $record, $length);
        $data        = pack("vvvv", $dxRwGut, $dxColGut, $maxRowOutlinetudiantl, $col_level);

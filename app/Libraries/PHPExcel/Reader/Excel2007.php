                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if (isset($xmlSheet->cols) && !$this->readDataOnly) {
                                foreach ($xmlSheet->cols->col as $col) {
                                    for ($i = intval($col["min"]) - 1; $i < intval($col["max"]); ++$i) {
                                        ...
                                        if ($col["outlinetudiantl"] > 0) {
                                            $docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setOutlinetudiantl(intval($col["outlinetudiantl"]));
                                        }
                            ...
                            if ($xmlSheet && $xmlSheet->sheetData && $xmlSheet->sheetData->row) {
                                foreach ($xmlSheet->sheetData->row as $row) {
                                    ...
                                    if ($row["outlinetudiantl"] > 0) {
                                        $docSheet->getRowDimension(intval($row["r"]))->setOutlinetudiantl(intval($row["outlinetudiantl"]));
                                    }

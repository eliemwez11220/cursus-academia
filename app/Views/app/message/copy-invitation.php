                                            ...
                                            if (isset($parents) && !empty($parents)):
                                                foreach ($parents as $key => $value): ?>
                                                    <option value="<?= esc($value['parent_uid']); ?>" <?= set_select('parent', esc($value['parent_uid'])); ?>>
                                                        <?= ucfirst(esc($value['parent_nom_tuteur'])); ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                        </select>
                                    ...
                                    <div class="col-sm-8">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" placeholder="Ex: avis de communiqué" name="objet_message"
                                                   id="objet_message" value="<?= (isset($message) ? ($message['message_objet']) : 'Aucun') ?>"/>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <textarea id="composemessage" name="contenu_message" class="form-control"
                                                      rows="10" style="height: 300px!important;"
                                                      placeholder="Decrivez le contenu du message ici..."><?= old('contenu_message'); ?><?= (isset($message) ? ($message['message_contenu']) : 'Aucun') ?></textarea>
                                        </div>
